...
def search_for_movies():
    ...
    excluded_urls = [
        "https://filmow.com/filmes/",
        "https://filmow.com/filmes-acao/",
        "https://filmow.com/filmes-reality-show/"
    ]
    links_elements = browser.find_elements(By.XPATH, '//*[@id="sidebar-genres"]/li[*]/a')
    links = []
    movies_data = []
    ...
    for link in links:
        browser.get(link) # "?order=newer"
        movies_data.extend(list_movies(browser, 1))

    wait_until_load(browser)
    browser.quit()

    file = open('output.json', 'w')
    file.write(json.dumps(movies_data))
    file.close()
